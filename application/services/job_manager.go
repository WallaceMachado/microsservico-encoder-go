...
import (
	...
	"github.com/wallacemachado/microsservico-encoder-go/framework/queue"
)
...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	// quantidade processos de videos que serão realizados em concorrência/simultaneos
...
func (j *JobManager) notifySuccess(jobResult JobWorkerResult, ch *amqp.Channel) error {
	...
	// infomra que a mensagel chegou certa e não teve erro, false para que não volte pra fila
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	...
	// rejeitando a mensagem pois teve algum erro, false para que não volte pra fila
